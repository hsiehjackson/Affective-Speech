...
class ClassifyDataset(Dataset):
    def __init__(self, h5_path, index_path, speaker_info_path, emotion, segment_size=128):
        ...
        for speaker_id in self.class_indexes:
            if self.speaker_emotions[speaker_id] == emotion or self.speaker_emotions[speaker_id] == 'neu':
                self.total_indexes += [speaker_id]

        print('Total Utterances {} + neu: {}'.format(emotion, len(self.total_indexes)))
    ...
    def __getitem__(self, index):
        ...
        label = 0 if self.speaker_emotions[speaker_id] == 'neu' else 1
        
        data = {
            'speaker': speaker_id,  
            'mel': self.dataset[f'{speaker_id}/mel'][:],
            'label': label
        }
...
class ClassifyTestDateset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        return {'speaker': self.speakers[index], 'tar': data_mel, 'src': random_mel, 'label': label}
