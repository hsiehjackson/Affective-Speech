...
from .hyperparams_AdaVAE_d import Hyperparams as hp
import numpy as np
...
def wav2wav(inpath,outpath):
    wav = librosa.core.load(inpath, sr=hp.sr)[0]
    wav, index = librosa.effects.trim(wav, top_db=hp.top_db)
    write(outpath, rate=hp.sr, data=wav)
...
def melspectrogram2wav(mel, path):
    ...
    write(path, rate=hp.sr, data=wav)
